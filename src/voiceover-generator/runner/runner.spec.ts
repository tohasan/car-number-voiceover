    ...
    describe('#run', () => {
        ...
        it('should translate car numbers into voiceovers', () => {
            ...
            expect(voiceovers).toEqual([
                'Х002СХ 199;Харитон два ноля двойка сэ Христофор регион сто девяносто девять',
                'Х002СХ 199;х дубль ноль двойка Светлана Харитон регион один дуплет девяток',
                'Х002СХ 199;ха два нуля два эс ха регион единица дуплет девяток',
                'М999ММ 47;Михаил три девятки дуплет м регион сорок семь',
                'М999ММ 47;м триплет девяток дуплет мэ регион четверка семерка',
                'М999ММ 47;эм девятьсот девяносто девять два эм регион четверка семь',
                'Е003АЕ 99;Егор два ноля тройка Анна Егор регион дуплет девяток',
                'Е003АЕ 99;Елена дубль ноль тройка Александр Егор регион дубль девять',
                'Е003АЕ 99;е два нуля три а е регион девяносто девять'
            ].join('\n'));
        });
        ...
        it('should keep generate different combinations for hundreds of car numbers', () => {
            ...
            expect(carNumber088).toEqual([
                'В088РВ 47;вэ ноль восемьдесят восемь эр вэ регион сорок семь',
                'В088РВ 47;в нуль две восьмерки рэ в регион четыре семь',
                'В088РВ 47;Владимир ноль дубль восемь р Владимир регион четверка семерка'
            ]);
        });
        ...
        it('should get voiceovers for latvian car numbers', () => {
            ...
            expect(voiceovers).toEqual([
                ...
                'LV NX-1133;л вэ пробел н ха дефис сто тринадцать тройка'
            ].join('\n'));
        });
        ...
        it('should get voiceovers for finnish car numbers', () => {
            ...
            expect(voiceovers).toEqual([
                ...
                'FIN EGT-018;эф ай латинская эн пробел е гэ тэ дефис нуль восемнадцать',
                'FIN EGT-018;страна фэ и с точкой английская эн латинская е латинская гэ т дефис ноль один восемь'
            ].join('\n'));
        });
